...
function Console:Window(consoledebugger)
    ...
    local GuiPosition = consoledebugger.Position or UDim2.new(0.5, 0, 0.5, 0)
    local DragSpeed = consoledebugger.DragSpeed or 0.25
    ...
    STX_Console.Parent = game:GetService("CoreGui")
    ...
    local function Update(dt)
        ...
        if not dragging and lastGoalPos then
            ambientShadow.Position = UDim2.new(
                startPos.X.Scale, 
                Lerp(ambientShadow.Position.X.Offset, lastGoalPos.X.Offset, dt * DRAG_SPEED), 
                startPos.Y.Scale, 
                Lerp(ambientShadow.Position.Y.Offset, lastGoalPos.Y.Offset, dt * DRAG_SPEED)
            )
            return 
        end
        ...
        ambientShadow.Position = UDim2.new(
            startPos.X.Scale, 
            Lerp(ambientShadow.Position.X.Offset, xGoal, dt * DRAG_SPEED), 
            startPos.Y.Scale, 
            Lerp(ambientShadow.Position.Y.Offset, yGoal, dt * DRAG_SPEED)
        )
    end

    ambientShadow.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            dragStart = input.Position
            startPos = ambientShadow.Position
    ...
    ambientShadow.InputChanged:Connect(function(input)
    ...
    ConsoleContainer.Size = UDim2.new(0, 580, 0, 396)
    ...
    AutoScroll_Locked_Title.Font = Enum.Font.GothamMedium
    ...
    AutoScroll_Locked_Title_2.Font = Enum.Font.GothamMedium
    ...
    TextBox.Position = UDim2.new(0.0166666675, 0, 0.916000009, 0)
    TextBox.Size = UDim2.new(0, 580, 0, 31)
    TextBox.Font = Enum.Font.Code
    TextBox.PlaceholderColor3 = Color3.fromRGB(220, 220, 220)
    ...
    -- Toggle visibility with RightAlt
    ...
    -- Create the ConsoleLog object that will be returned
    local ConsoleLog = {}
    local CommandHandlers = {}

    -- Add command registration function with optional description parameter
    -- Usage:
    --   ConsoleLog:AddCommand("name", function(console, ...)
    --       -- handler
    --   end)
    --   ConsoleLog:AddCommand("name", "Description here", function(console, ...)
    --       -- handler
    --   end)
    function ConsoleLog:AddCommand(commandName, arg2, arg3)
        local description, handlerFunction
        if type(arg2) == "string" and type(arg3) == "function" then
            description = arg2
            handlerFunction = arg3
        elseif type(arg2) == "function" and arg3 == nil then
            handlerFunction = arg2
            description = "Custom command"
        else
            error("Invalid parameters for AddCommand. Expected (name, func) or (name, description, func)")
        end
        CommandHandlers[commandName:lower()] = {
    ...
    ConsoleLog:AddCommand("help", function(console)
        ...
        console:Prompt({
            Title = "print [text] - Prints text to console",
            Type = "default"
        })
        -- List other registered commands
        for cmd, data in pairs(CommandHandlers) do
            if cmd ~= "help" and cmd ~= "clear" and cmd ~= "print" then
                console:Prompt({
                    Title = cmd .. " - " .. (data.description or "Custom command"),
                    Type = "default"
                })
            end
        end
    ...
    function ConsoleLog:Prompt(promptdebugger)
        ...
        local Type = (string.lower(tostring(promptdebugger.Type or "default")))
    ...
    TextBox.FocusLost:Connect(function(enterPressed)
        if enterPressed then
            ...
            if #parts == 0 then return end -- Empty command
            
            local commandName = parts[1]:lower()
            table.remove(parts, 1) -- Remove command name, leaving just arguments
            
            -- Check for built-in commands
            if commandName == "clear" then
                -- Clear console
                for _, child in ipairs(ConsoleContainer:GetChildren()) do
                    if child:IsA("TextLabel") then
                        child:Destroy()
                    end
                end
            elseif commandName == "print" then
                -- Handle print command
                local textToPrint = table.concat(parts, " ")
                if textToPrint ~= "" then
                    ConsoleLog:Prompt({
                        Title = textToPrint,
                        Type = "nofitication"
                    ...
                    ConsoleLog:Prompt({
                        Title = "Error: Nothing to print",
                        Type = "fail"
                    })
                end
            elseif CommandHandlers[commandName] then
                -- Handle registered command
                local success, err = pcall(function()
                    CommandHandlers[commandName].handler(ConsoleLog, unpack(parts))
                end)
                if not success then
                    ConsoleLog:Prompt({
                        Title = "Error executing command: "..err,
                        Type = "fail"
                    })
                end
            else
                -- Unknown command
                ConsoleLog:Prompt({
                    Title = "Error: Unknown command '"..command.."' (type 'help' for commands)",
                    Type = "fail"
                })
            end
        end
