        ...
        function Library:Draggify(Parent)
            ...
            Library:Connection(InputService.InputChanged, function(Input, game_event) 
                if Dragging and Input.UserInputType == Enum.UserInputType.MouseMovement then
                    ...
                    local maxX = Horizontal - Parent.Size.X.Offset
                    local maxY = Vertical - Parent.Size.Y.Offset
                    if maxX < 0 then maxX = 0 end
                    if maxY < 0 then maxY = 0 end
                    local newX = math.clamp(InitialSize.X.Offset + (Input.Position.X - InitialPosition.X), 0, maxX)
                    local newY = math.clamp(InitialSize.Y.Offset + (Input.Position.Y - InitialPosition.Y), 0, maxY)
                    local NewPosition = dim2(0, newX, 0, newY)
                    Library:Tween(Parent, {
        ...
        function Library:Keypicker(properties) 
            ...
            function Cfg.UpdateColor() 
                ...
                local function safeClamp(val, min, max)
                    if type(val) ~= "number" or type(min) ~= "number" or type(max) ~= "number" then return 0 end
                    if max < min then max = min end
                    return math.clamp(val, min, max)
                end

                if DraggingSat and Items.SatValHolder and Items.SatValHolder.AbsolutePosition and Items.SatValHolder.AbsoluteSize then
                    local denomX = Items.SatValHolder.AbsoluteSize.X
                    local denomY = Items.SatValHolder.AbsoluteSize.Y
                    if denomX and denomX ~= 0 then
                        s = 1 - safeClamp((offset - Items.SatValHolder.AbsolutePosition).X / denomX, 0, 1)
                    end
                    if denomY and denomY ~= 0 then
                        v = 1 - safeClamp((offset - Items.SatValHolder.AbsolutePosition).Y / denomY, 0, 1)
                    end
                elseif DraggingHue and Items.Hue and Items.Hue.AbsolutePosition and Items.Hue.AbsoluteSize then
                    local denom = Items.Hue.AbsoluteSize.X
                    if denom and denom ~= 0 then
                        h = safeClamp((offset - Items.Hue.AbsolutePosition).X / denom, 0, 1)
                    end
                elseif DraggingAlpha and Items.Alpha and Items.Alpha.AbsolutePosition and Items.Alpha.AbsoluteSize then
                    local denom = Items.Alpha.AbsoluteSize.Y
                    if denom and denom ~= 0 then
                        a = safeClamp((offset - Items.Alpha.AbsolutePosition).Y / denom, 0, 1)
                    end
                end
    ...
    -- Library element functions
        ...
        function Library:Slider(properties) 
            ...
            function Cfg.Set(value)
                -- Sanity checks for slider values
                if type(value) ~= "number" then value = Cfg.Min or 0 end
                if Cfg.Min and Cfg.Max and Cfg.Max < Cfg.Min then Cfg.Max = Cfg.Min end
                local min, max = Cfg.Min or 0, Cfg.Max or 1
                local intervals = Cfg.Intervals or 1
                local rounded = Library:Round(value, intervals)
                Cfg.Value = math.clamp(rounded, min, max)
                Items.Value.Text = tostring(Cfg.Value) .. (Cfg.Suffix or "")
                local denom = (max - min)
                local percent = denom ~= 0 and (Cfg.Value - min) / denom or 0
                Library:Tween(Items.Accent, {Size = dim2(percent, 0, 1, 0)}, TweenInfo.new(Library.DraggingSpeed, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, 0, false, 0))
